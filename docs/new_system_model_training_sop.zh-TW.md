...
正規化尺度只由 **OK 圖**決定。沒有另外提供 OK 校準圖時，系統從各子模型的 OK 圖保留 20% 作為校準資料，其餘建立正常記憶庫。
**NG Bomb 只用於模型完成後的抓取率與品質評估，不影響正規化尺度**；也不再產生合成 NG 來決定尺度。
分數改用 **原始距離 ÷ OK 校準最高值 × 0.5**；總分與熱圖各用自己的 OK 校準最高值。正的小分數保留，超過 1 也保留；熱圖只在上色時將超過 1 顯示為最紅。原始距離為 0 或遮罩排除的位置仍可為 0，低分熱圖仍可能偏藍。
新模型預設門檻為 **0.5**。舊模型須重訓才會更新算法；完整訓練、局部重訓或單一子模型重訓都使用新算法。第一次改用新算法的子模型，原門檻會同步重設為 0.5；之後同算法重訓會保留操作員設定的門檻。重訓完成後請用現場 OK／NG 樣本確認門檻。
