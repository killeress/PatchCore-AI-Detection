...
注意: 圖片路徑可能包含分號時，Server 會把第 6 欄之後重新 join 成路徑。但客戶端規格仍建議避免在路徑中使用 `;`。
...
5. 若 `機檢判定 == HY`，跳過 AI 推論，直接回覆畫異 defect。
6. 其他情境執行 AI 推論，產出 `OK` / `NG` / `OK-i` / `ERR:*`。
7. 先回覆客戶端，再非同步寫入 DB、heatmap、推論 log。
