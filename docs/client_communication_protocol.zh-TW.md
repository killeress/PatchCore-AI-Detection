...
- `White_Frame`（含 `BWFRAME0` 別名）座標除了整片白框檢測，還會在原始解析度的 AOI TILE 檢查局部白線缺口及變細；局部 NG 以 `WHITEFRA` 畫面回報。
- 同一產品座標會追加到 `W0F00000`，沿用白畫面 PatchCore 模型、邊緣 TILE 向內修正及後處理。此補檢的缺陷以 `W0F00000` 畫面回報，原始 AOI 座標仍保留在 `WHITEFRA`。
- White_Frame 局部檢查或 W0F00000 補檢任一出現有效 NG，整片 ID 保留 NG，不再經規格內判定轉成 `OK-i`；PatchCore 的有效 NG 仍依既有後處理結果判斷。
- 推論紀錄的白框觀察區塊右側顯示 OpenCV 放大檢查，下方顯示白畫面 PatchCore 分數、座標修正、熱力圖與樣本資料；缺少白畫面結果時會顯示提示。CAPI 不追加這兩項檢查。
