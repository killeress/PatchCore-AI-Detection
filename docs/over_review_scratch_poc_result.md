...
# patch-level pooling：--pool-mode cls|max|cls_max|cls_max_anom|cls_top5_anom
python -m scripts.over_review_poc.run_poc --manifest ... --output ... \
    --transform clahe --clahe-clip 4.0 --pool-mode cls_top5_anom

# 找 deployment threshold（conformal calibration）
python -m scripts.over_review_poc.zero_leak_analysis \
    --embeddings reports/poc_cl4_clsanom/embeddings_cache.npz \
    --manifest datasets/over_review/manifest.csv --safety 1.05

# LoRA fine-tune DINOv2 最後 2 block（破 Oracle 天花板）
...
### Patch-level pooling ablation（all with CLAHE cl=4）

| Pool mode | Realistic | true_ng | Oracle | Embed dim | commit |
|---|---|---|---|---|---|
| cls (baseline) | 87.6% | 96.2% | 65.9% | 768 | `a3f2d16` |
| max (channel-wise) | 85.9% | 95.3% | 72.7% | 768 | `a3f2d16` |
| cls_max | 87.7% | 95.9% | 73.7% | 1536 | `a3f2d16` |
| cls_max_anom | 88.5% | 96.2% | **73.8%** | 1536 | `a3f2d16` |
| **cls_top5_anom** | **91.1%** | 95.8% | 71.9% | 1536 | `a3f2d16` |

> `*_anom` 模式 rank patches by L2 距離 image-mean → top-5 patch 捕捉 scratch 區域。

### LoRA fine-tune（破 Oracle 天花板）

| 配置 | Realistic | true_ng | Oracle | PR-AUC | 漏檢 |
|---|---|---|---|---|---|
| LoRA r=8 / 2 blocks / 10 ep + cl=4 | 89.4% ± 6.0% | 98.3% ± 1.5% | 80.8% ± 12.4% | 0.937 | 1.7% |
| **LoRA r=16 / 2 blocks / 15 ep + cl=4** | **90.4% ± 5.8%** | **98.2% ± 1.7%** | **86.0% ± 9.2%** 🎯 | 0.931 | **1.8%** |

r=16 的 fold 1 達成 Realistic 90.9% / **true_ng 100.0%**（零漏檢）、fold 3 全 100%。Oracle 86% 意為「若能找到對的 threshold，可在零漏檢前提下擋下 86% scratch」— 相較 v1 baseline Oracle 66.7% 大幅 +19%。
...
- Realistic **90.4%** / true_ng **98.2%** / **Oracle 86.0%** / PR-AUC 0.931
- 相對 v1 baseline：Realistic +2.7%、Oracle **+19.3%**、漏檢從 3.3% 降到 **1.8%**

### 下一步建議

| 優先 | 項目 |
|---|---|
| 1 | **LoRA + patch-level pool**（結合 cls_max_anom）嘗試再推 Oracle |
| 2 | **Conformal threshold on LoRA 特徵** 驗證 0-leak deployment 能達到多少 |
| 3 | 蒐集 W0F/WGF/STANDARD 光源樣本（目前各 <10 筆） |
| 4 | **兩階段系統**：LoRA auto-classify + borderline 人工覆判（achieves 真 0% leak） |
| 5 | 替 edge_defect scratch 寫專屬幾何判別（非 ML） |
