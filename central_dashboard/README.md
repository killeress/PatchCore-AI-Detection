...
看板頂部可切換「即時 | 歷史班報」。歷史班報模式只顯示查詢列與結果表（線體 / AOI 排片率 / AI 排片率 / 該班投入），不顯示即時狀態、AOI 連線、最近生產活動、目前告警與硬體資訊。

- 查詢條件：日期（最近 29 天，更早的日期因線體 OK 紀錄只保留 30 天而失真，選擇器直接禁用）＋班別（白班 07:30～19:30；晚班 19:30～隔日 07:30，歸屬起班日，畫面顯示完整起訖）。
- 僅能查詢已結束的班別；尚未結束的組合會被禁用，請改用即時模式查看。
- 線體範圍：列出設定中的全部線體（含停用），仍套用廠區網段過濾；日期或班別變更即自動重查，逾時沿用看板的 API 逾時設定。
- 口徑與即時模式一致：該班投入 = OK + NG + ERR；AOI／AI 排片率分母同為總投入。投入為 0 時排片率顯示「—」。
- 資料由各線 PC 即時回傳：查詢當下離線的線體顯示「離線無法查詢」；舊版線體（無此 API）顯示「未更新，請更新線體程式」，更新後即可查詢。舊版線體的 404 回應不帶 CORS 標頭、瀏覽器讀不到狀態碼，故查詢失敗時看板會改探該線的 `/api/status` 來區分「未更新」與「離線」。
- 部署前提：各線 `cleanup.ok_retain_days` 需 ≥ 30（缺少此設定的線體預設只留 14 天，查 14 天前的班報會失真）。

### 線體端 API

```text
GET /api/shift_report?date=2026-01-20&shift=day
```

- `shift`：`day`（白班）或 `night`（晚班）。
- 唯讀、免登入，回應標頭含 `Access-Control-Allow-Origin: *`（比照 `/api/status`，備援的 `file://` 模式亦可查詢）。
- 回傳：`date`、`shift`、`shift_name`、`time_range`、`start`、`end`、`total`、`ok_count`、`ng_count`、`aoi_ng_count`、`err_count`；參數缺漏或格式錯誤回 400。
