...
def test_v2_omit_postprocess_masks_aoi_exclude_zone_before_two_stage():
    config = CAPIConfig()
    config.dust_two_stage_enabled = True
    config.dust_heatmap_top_percent = 0.4
    ...
    inferencer.edge_inspector = SimpleNamespace(
        config=EdgeInspectionConfig(
            exclude_zones=[
                EdgeExclusionZoneConfig(enabled=True, x=8, y=0, w=8, h=16)
            ]
        )
    )
    ...
    calls = {"per_region": False, "two_stage": False, "debug": False}
    dust_mask = np.full((16, 16), 255, dtype=np.uint8)
    dust_mask_no_ext = np.zeros((16, 16), dtype=np.uint8)

    def assert_exclude_zone_zeroed(amap):
        assert np.all(amap[:, 4:] == 0)
        assert amap[4, 2] == 20.0
    ...
    def fake_check_dust_per_region(_dust_mask, amap, **_kwargs):
        calls["per_region"] = True
    ...
    def fake_check_dust_two_stage(
        ...
        *,
        ...
        return (
            True,
            (4, 2),
            [{"abs_pos": (4, 8), "area": 4, "is_dust": False}],
            "TWO_STAGE: 1real+0dust -> REAL_NG",
        )

    def fake_generate_two_stage_debug_image(_tile_image, amap, dm, features, is_dust):
        calls["debug"] = True
        assert dm is dust_mask_no_ext
        assert is_dust is False
        assert features == [{"abs_pos": (4, 8), "area": 4, "is_dust": False}]
        assert_exclude_zone_zeroed(amap)
        return np.zeros((4, 4, 3), dtype=np.uint8)
    ...
    assert calls == {"per_region": True, "two_stage": True, "debug": True}
    assert tile.is_suspected_dust_or_scratch is False
    assert tile.anomaly_peak_x == 4
    assert tile.anomaly_peak_y == 8
    assert "EXCLUDE_ZONE_HEATMAP_ZEROED" in tile.dust_detail_text
    assert "TWO_STAGE: 1real+0dust -> REAL_NG" in tile.dust_detail_text
