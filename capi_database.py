...
class CAPIDatabase:
    ...
    def init_config_from_yaml(self, config) -> int:
        ...
        params_def = [
            ...
            # CV 邊緣檢測
            ("cv_edge_light_leak_enabled", False, "bool", "啟用邊緣漏光檢測（AOI 灰塵判定後，以四邊連續亮帶救回 NG；獨立於 CV 主開關）"),
            ...
            ("cv_edge_light_leak_min_length", 30, "int", "邊緣漏光檢測：亮帶最小連續長度（產品座標 px）"),
            ...
            ("cv_edge_light_leak_max_dust_overlap", 0.2, "float", "邊緣漏光檢測：候選亮帶允許的最大 OMIT 灰塵覆蓋率（0~1）"),
