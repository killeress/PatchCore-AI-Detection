...
class CAPIDatabase:
    ...
    def init_config_from_yaml(self, config) -> int:
        ...
        params_def = [
            ...
            ("scratch_classifier_enabled", config.scratch_classifier_enabled, "bool", "啟用刮痕分類器"),
            ("scratch_safety_multiplier", config.scratch_safety_multiplier, "float", "刮痕判定安全倍率"),
            ("scratch_bundle_path", config.scratch_bundle_path, "string", "刮痕分類器模型檔路徑"),
            ("scratch_dinov2_repo_path", config.scratch_dinov2_repo_path, "string", "DINOv2 本地 repo 路徑"),
            ("scratch_dinov2_weights_path", config.scratch_dinov2_weights_path, "string", "DINOv2 權重檔路徑"),
            ...
            ("dust_mask_before_binarize", config.dust_mask_before_binarize, "bool", "先遮罩灰塵區域再做 top% 二值化"),
            ("dust_two_stage_enabled", config.dust_two_stage_enabled, "bool", "啟用兩階段精準比對"),
            ("dust_two_stage_dust_ratio", config.dust_two_stage_dust_ratio, "float", "特徵點與灰塵重疊比例（0~1）"),
            ("dust_two_stage_bg_blur", config.dust_two_stage_bg_blur, "int", "局部背景估計 Gaussian Blur 核大小（奇數 px）"),
            ("dust_two_stage_diff_percentile", config.dust_two_stage_diff_percentile, "float", "特徵偵測 diff 分布百分位"),
            ("dust_two_stage_min_area", config.dust_two_stage_min_area, "int", "特徵最小面積（px）"),
            ("dust_detect_dark_particles", config.dust_detect_dark_particles, "bool", "偵測暗色顆粒/圖案 (如偏黑 MARK) 並過濾"),
            ("dust_detect_bubbles_enabled", config.dust_detect_bubbles_enabled, "bool", "偵測低對比氣泡/大片暗斑"),
            # Otsu 邊緣裁切
        ...
        description_refresh_param_names = image_abnormal_param_names | {
            ...
            "dust_two_stage_enabled",
            "dust_two_stage_dust_ratio",
            "dust_two_stage_bg_blur",
            "dust_two_stage_diff_percentile",
            "dust_two_stage_min_area",
